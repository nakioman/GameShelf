    ...
    <Modal onClose={closeManual} className="w-[95%] max-w-[900px] h-[90vh] flex flex-col">
      <div className="bg-shelf-panel rounded-xl overflow-hidden flex flex-col h-full">
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-shelf-bg border-b-2 border-shelf-accent">
          <h3 className="text-shelf-accent font-semibold">{t('manual.title')}</h3>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setPage(p => Math.max(1, p - 1))}
              className="px-3 py-1 bg-shelf-card text-shelf-text rounded text-sm hover:bg-shelf-accent hover:text-white transition">{t('manual.prev')}</button>
            <span className="text-sm text-shelf-text-dim">{t('manual.page', { page, total })}</span>
            <button onClick={() => setPage(p => Math.min(total, p + 1))}
              className="px-3 py-1 bg-shelf-card text-shelf-text rounded text-sm hover:bg-shelf-accent hover:text-white transition">{t('manual.next')}</button>
          </div>
        ...
        <div className="flex-1 overflow-auto flex items-center justify-center p-5">
          {pdfDoc ? (
            ...
            <p className="text-shelf-text-dim">{t('manual.loading')}</p>
          )}
        </div>
      </div>
    </Modal>
